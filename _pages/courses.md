---
layout: page
permalink: /courses/
title: courses undertaken
description: This is a descriptive compilation of the mathematical, statistical, and computer science courses I have taken.
nav: false
nav_order: 6
toc:
  sidebar: left
---

<h5>Computer Science</h5>
<ul>
  <li>Data Structures</li>
  <li>Web and Internet Programming</li>
  <li>Database Systems</li>
  <li>Software Engineering Principles</li>
  <li>Computer Architecture</li>
  <li>Algorithms</li>
  <li>Operating Systems</li>
  <li>Human-Computer Interaction (Current Enrollment)</li>
  <li>Shader Programming and Procedural Modeling (Current Enrollment)</li>
  <li>Software Engineering Practicum (Current Enrollment)</li>
</ul>

<h5>Mathematics</h5>
<ul>
  <li>Calculus and Analytical Geometry I & II</li>
  <li>Calculus III</li>
  <li>Discrete Mathematics</li>
  <li>Linear Algebra</li>
  <li>Modern Algebra</li>
</ul>

<h5>Mathematics, Statistics, and Computer Science</h5>
<ul>
  <li>Co-op: National Center for Atmospheric Research</li>
  <li>Formal Languages and Finite Automata</li>
  <li>Mathematical Modeling</li>
</ul>

<h5>Other</h5>
<ul>
  <li>Cybertechnology Ethics</li>
  <li>Probability and Mathematical Statistics I & II</li>
</ul>
